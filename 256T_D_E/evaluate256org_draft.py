...
tokensfile = "./data/tokens_dl.tsv"
# tokensfile = "./data/tokens_new.tsv"
...
#    ids_dist = [0 for i in range(118)]
    ...
    for d in data_loader:
        ...
        # f1.close()
            # hypotheses[0]["sequence"]["full"]
        tensor = hypotheses[0]["sequence"]["full"] #hypotheses[0]["sequence"]["full"][0]是每次循环输出第一个tensor,这里一次放进去batchsize个。
#        tf.constant([1,2,3]).eval()
        for sequence in tensor.cpu().numpy():
            ...
            for i in ids_:
                ids_dist[i]+=1
            # print(tokens_dist)   出来batchsize个，每个都累加新的sequence数字。
        # print(ids_dist)    #出来一个，统计batchsize个seq的token分布。
