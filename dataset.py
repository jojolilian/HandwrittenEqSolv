...
class CrohmeDataset(Dataset):
    ...
    def __init__(
        ...
        #     ] #old version, can't deal with the situation,when the groundtruth of expression has more than one \t
        with open(groundtruth, "r") as fd:
            reader =  fd.readlines()
                       
             
            self.data =                           [      {
                                "path": os.path.join(root, line.split("\t",1)[0] + ext), #only split once with \t
                                "truth": {
                                        "text": line.split("\t",1)[1],
                                        "encoded": [
                                                self.token_to_id[START],
