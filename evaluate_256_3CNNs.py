    ...
    if strip_only:
        ...
        for tok in tokens:
            if tok not in special_tokens:
                break
            num_left += 1
        for tok in reversed(tokens):#built in func inverse
            if tok not in special_tokens:
                break
            num_right += 1
            ...
            for hypothesis in hypotheses:
                ...
                for top_prob, top_id in zip(topk_probs.t(), topk_ids.t()):####.t() transpose
                    next_sequence = torch.cat(
                        (curr_sequence, top_id.view(-1, 1)), dim=1  #### view() represents resize , -1 : depends on other numbers。
                    )####cat（） concatnate  dim = 1 horizontal  dim = 0 verticle
...
#       
        ...
        num_tokens = {
            "full": expected.numel(),###numel() returns number of elements
