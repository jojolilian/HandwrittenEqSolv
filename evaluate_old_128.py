...
from model_old_128 import Encoder, Decoder
    ...
    if strip_only:
        ...
        for tok in tokens:
            if tok not in special_tokens:
                break
            num_left += 1
        for tok in reversed(tokens):#built in func 反向
            if tok not in special_tokens:
                break
            num_right += 1
...
def create_markdown_tables(results):
    ...
    err_table = "\n".join([err_header, err_delimiter, *err_values])# * 表示多个参数
...
def unbatch_hypotheses(hypotheses):
    ...
    for h in hypotheses:
        for i in range(len(h["probability"])):
            ...
            hypotheses_by_seq[i].append(single_h)#####这里的i指的是batch里的equation个数（即220行计算的prob有几个，就是一个batch中有几个equation）
    return hypotheses_by_seq#### 这里把输入（很多个分别包含一个batch equations的预测 变成了每个equation对应的很多不同预测）
...
def calculate_distances(hypothesis):
    sequence = hypothesis["sequence"]
    expected = hypothesis["expected"]
    distances_full = calc_distances(sequence["full"], expected["full"])
    distances_removed = calc_distances(sequence["removed"], expected["removed"])
    distances_symbols = calc_distances(sequence["symbols"], expected["symbols"])
    return distances_full,distances_removed,distances_symbols
...
def evaluate(
    ...
    batch_number = 0
    length_pred_distribute = [[ 0 for i in range(100)] for j in range(3)]
    ...
    for d in data_loader:
        ...
        for i in range(batch_max_len - 1):
            ...
            for hypothesis in hypotheses:
                ...
                topk_probs, topk_ids = torch.topk(probabilities, beam_width)######top k() 在probabilities中找出beamwidth个最大值
                ...
                for top_prob, top_id in zip(topk_probs.t(), topk_ids.t()):####.t() transpose
                    next_sequence = torch.cat(
                        (curr_sequence, top_id.view(-1, 1)), dim=1  #### view() 相当于 resize -1 表示 由其它数字决定。
                    )####cat（） concatnate  dim = 1 横着  dim = 0 竖着
...
##########################################################        
        ...
        '''
        output the length of prediction seq into length_pred_distribute
        '''
...
#        print('-----------------predicted length----------------------')
        for n,category in enumerate(batch_pred):
            for m,pred in enumerate(category):
                length_seq = np.size(batch_pred[n][m],0)
...
#        print(num_tokens["full"])
#        print(num_tokens["removed"])
#        print(num_tokens["symbols"])
        
        for hypothesis in hypotheses:
            sequence = hypothesis["sequence"]
            #print('预测值(full):'+str(hypothesis["sequence"]["full"][0]))##输出为数字矩阵 batchsize =4 所以为4个pred
            #print('实际值(full):'+str(expected[0]))#.id_to_token))                            #同样为4*x矩阵，x不确定
            sequence["removed"] = [
        ...
        '''
        output the distances  into distance_distribute
        '''       
        
        dist_list = [0,0,0]
        dist_list[0],dist_list[1],dist_list[2] = calculate_distances(hypotheses[0])
        for n,dist_batch in enumerate(dist_list):
            for length_of_distance in dist_batch:
                distance_distribute[n][length_of_distance] = distance_distribute[n][length_of_distance] +1
                
             
        '''
          output the actual length into length_actu_distribute
        '''
        length_list = [0,0,0]
        length_list[0]= expected.tolist()
        length_list[1]= [x.tolist() for x in expected_removed]
        length_list[2]= [x.tolist() for x in expected_symbols]
        
        
        for n,length_batch in enumerate(length_list):
            for m,seq in enumerate(length_batch):
                length_seq = np.size(length_list[n][m],0)
                num_pad = np.sum(np.array(seq) == 118)
                length_seq = length_seq - num_pad
                length_pred_distribute[n][length_seq] = length_pred_distribute[n][length_seq] +1

                length_actu_distribute[n][int(length_seq)] = length_actu_distribute[n][int(length_seq)] +1
#                print(seq)
#        print('-----------------predicted length----------------------')
