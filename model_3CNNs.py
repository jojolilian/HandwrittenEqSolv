...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        '''
        I changed the CNN in the input parts to see if it has a better performance than the old model 
        that only has 1 CNN before the denseblock
        
        The original CNN(conv0) here is basically replaced by 3 different CNN: conv0_1,conv0_2,conv0_3
        I assume that the deeper CNN may extract the patterns in the images better.
...
#        
        ...
        self.norm0_3 = nn.BatchNorm2d(num_in_features)
        self.relu = nn.ReLU(inplace=True)
        self.max_pool3 = nn.MaxPool2d(kernel_size=2, stride=2)
        
        '''
        The cnn above is the changed part
        First CNN: 9X9 padding 4
        Second CNN: 7X7 padding 3
        Third CNN: 1X1 padding 0    CCCP(MLPCNN)
        '''
    ...
    def forward(self, x):
        ...
        '''
        Here are the changes based on the changes above(3 different CNNs)
        '''
        out = self.conv0_1(x)
        out = self.relu(self.norm0_1(out))
#        out = self.max_pool1(out)
        out = self.conv0_2(out)
        out = self.relu(self.norm0_2(out))
#        out = self.max_pool2(out)
        out = self.conv0_3(out)
        out = self.relu(self.norm0_3(out))
