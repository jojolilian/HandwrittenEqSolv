...
def encode_truth(truth, token_to_id):
    ...
    while len(remaining_truth) > 0:
        try:
            ...
            raise Exception("Truth contains unknown token。truth:"+str(remaining_truth[:50]))
    
    
    return truth_tokens #返回的是编完号的groundtruth
...
class CrohmeDataset(Dataset):
    ...
    def __init__(
        ...
        Args:
            ...
            crop (bool, optional): Crop images to their bounding boxes [Default: Falsle] 修剪
...
#                row = line.split("\t",1)
            ...
            self.data =                           [      {
                                "path": os.path.join(root, line.split("\t",1)[0] + ext),
                                "truth": {
                                        "text": line.split("\t",1)[1],
                                        "encoded": [
                                                self.token_to_id[START],
                                                *encode_truth(line.split("\t",1)[1], self.token_to_id),
...
#            truth_idset.append(self.data["truth"]["encoded"])  #出错，因为self.data是一堆dict组成的list不能这样访问其中的key对应的value，要递归访问
                
#        print("finished")   #说明没有unknown_token,都encode了。
#        print(self.data)  #没padding之前的encode,self.data 是一堆字典组成的list
