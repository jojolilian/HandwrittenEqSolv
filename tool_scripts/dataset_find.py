...
def encode_truth(truth, token_to_id):
    ...
    while len(remaining_truth) > 0:
        try:
            ...
            raise Exception("Truth contains unknown token. truth:"+str(remaining_truth[:50])) #first token and a litte context after it in the output shows the unknown token
